...
if (elgg_instanceof($page_owner, 'user')) {
	$url_segment = 'exercise/archive/' . $page_owner->username;
} else {
	$url_segment = 'exercise/group/' . $page_owner->getGUID() . '/archive';
}

// This is a limitation of the URL schema.
if ($page_owner && $vars['page'] != 'friends') {
	$dates = get_entity_dates('object', 'exercise', $page_owner->getGUID());
	
	if ($dates) {
		$title = elgg_echo('exercise:archives');
		$content = '<ul class="exercise-archives">';
